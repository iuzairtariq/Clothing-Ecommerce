...
const AdminOrders = ({ adminToken }) => {
  ...
  return (
    <div>
      <Title text1={'ORDER'} text2={'PRODUCTS'} />
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_1fr_1fr] gap-3 items-start border-2 border-slate-400 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-slate-600 dark:text-slate-400' key={index}>
              ...
              <div>
                ...
                <p className='mt-3 mb-1 font-bold'>{order.address.firstName + " " + order.address.lastName}</p>
                <div>
              ...
              <p className='text-center font-bold'>{currency} {order.amount}</p>
              <select onChange={(event) => statusHandler(event, order._id)} value={order.status} className='bg-accent p-2 font-semibold w-40 mx-auto' name="" id="">
